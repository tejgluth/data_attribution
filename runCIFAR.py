...
RESULTS_DIR = os.path.join(BASE_DIR, "resultsCIFAR")
...
LISSA_DEPTH = 30
LISSA_SCALE = 2000.0
#CIFAR has higher loss curvature than MNIST so we need a bigger scale to keep the LiSSA series convergent (eigenvalues of (I - H/scale) must stay <1 in magnitude)
#also bumped depth a bit so the estimate has more iterations to settle in
...
def load_data():
    transform = transforms.Compose([
        ...
        #per-channel mean and std for CIFAR-10 (R, G, B)
    ])
...
class SmallCNN(nn.Module):
    def __init__(self):
        ...
        #CIFAR images are 3-channel RGB (32x32) instead of 1-channel grayscale (28x28)
        self.conv2 = nn.Conv2d(CONV1_CHANNELS, CONV2_CHANNELS, kernel_size=3, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2)
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(CONV2_CHANNELS * 8 * 8, FC_HIDDEN_DIM)
        #after 2 maxpools on a 32x32 input we get 8x8 spatial dims (vs 7x7 for MNIST's 28x28)
...
def visualize(train_dataset, test_image, test_label, scores, verification_results, method_name, output_name):
    ...
    #CIFAR images are 3xHxW normalized tensors, so we need to undo the normalization and put channels last (HxWx3) for matplotlib to display them as RGB
    cifar_mean = np.array([0.4914, 0.4822, 0.4465]).reshape(3, 1, 1)
    cifar_std = np.array([0.2470, 0.2435, 0.2616]).reshape(3, 1, 1)

    def to_display(tensor):
        array = tensor.detach().cpu().numpy()
        array = array * cifar_std + cifar_mean #undo normalization so colors look right
        array = np.clip(array, 0.0, 1.0)
        return np.transpose(array, (1, 2, 0)) #CHW -> HWC for imshow

    def get_image(idx): #helpers to get the image and label for a specific index in the training dataset
        return to_display(train_dataset[idx][0])

    def get_label(idx):
        return train_dataset[idx][1]

    ax = fig.add_subplot(2, 7, 1)
    ax.imshow(to_display(test_image))
